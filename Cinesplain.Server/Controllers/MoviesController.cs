...
public class MoviesController(IConfiguration config) : Controller
{
    ...
    public ActionResult<MovieListPage> GetUpcomingMovies()
    {
        try
        {
            ...
            var movieList = ApiUtility.GetTMDBResponse<MovieListPage>(_config, $"discover/movie", queryParams);
    ...
    public ActionResult<MovieDetails> GetOmdbMovieDetails(string id)
    {
        try
        {
            var omdbMovieDetails = ApiUtility.GetOMDBResponse<MovieDetails>(_config, id);
